...
public interface AiResultDao extends JpaRepository<AiResult, Integer> {
    ...
    @Query(name = "查询所有数据",value = "select id, file_name,num, time, final_label,final_score,result, port, url, create_time from ai_result where port = ?1 and create_time between ?2 and ?3", nativeQuery = true)
